...
class SmoothnessNode:
    def __init__(self, factorID, lambdaIn, prevID, afterID):
        ...
        # has the same form
        self.lambdaIn = lambdaIn #shape (1,1)
        self.eta = np.matmul(J.T, lambdaIn) * 0 #shape(2,1) [[0], [0]]
        self.lambda_prime = np.matmul(np.matmul(J.T, lambdaIn), J) #shape(2,2)
        # print(J.T)
        # print(self.lambdaIn)
...
def logResult(dur):
    ...
    with open('resultLog/resultLog.csv', mode='a') as csv_file: # 'a' = append
        writer = csv.DictWriter(csv_file, fieldnames=FIELD_NAME)

        writer.writerow({'image': IMAGE_NAME, 'number_of_iter': ITER, 'duration_ms' : dur,
...
for i in range(rows):
    for j in range(cols):
        ...
        variableNodes[varId] = VariableNode(varId, np.array([[0.0]]).astype(np.float32),
                                            np.array([[0.0]]).astype(np.float32),
                                            varId, leftId, rightId, upId, downId)
        factorNodes[varId] = MeasurementNode(varId, noisySignal[i][j],
                                             np.array([[1.0 / SIGMAMeas ** 2]]).astype(np.float32), varId)
        if leftId != -1 and leftId not in factorNodes:
            factorNodes[leftId] = SmoothnessNode(leftId, np.array([[1.0 / SIGMASmooth ** 2]]).astype(np.float32),
                                                 min(leftId), max(leftId))
            # print(leftId) nothing printed
        if rightId != -1 and rightId not in factorNodes:
            factorNodes[rightId] = SmoothnessNode(rightId, np.array([[1.0 / SIGMASmooth ** 2]]).astype(np.float32),
                                                  min(rightId), max(rightId))
            # print(rightId)
        if upId != -1 and upId not in factorNodes:
            factorNodes[upId] = SmoothnessNode(upId, np.array([[1.0 / SIGMASmooth ** 2]]).astype(np.float32),
                                               min(upId), max(upId))
            # print(upId) nothing printed
        if downId != -1 and downId not in factorNodes:
            factorNodes[downId] = SmoothnessNode(downId, np.array([[1.0 / SIGMASmooth ** 2]]).astype(np.float32),
                                                 min(downId), max(downId))
            # print(downId)

# print(len(variableNodes))
# print(len(factorNodes))
#
# print(173280 in factorNodes) #False
# printGraph()
...
start = time.time()
while iters < ITER:
    if not EVAL_MODE : print('Iteration ', iters)
    ...
    iters += 1
    if RENDER_OUTPUT and useRealImage:
        result = old_mu.reshape((rows, cols))
        cv2.imwrite(resultFolder + 'result' + str(iters) + '.png', result)


duration = time.time() - start #second
if not EVAL_MODE: print('duration : ')
print(duration * 1000) # ms 


logResult(duration * 1000)
